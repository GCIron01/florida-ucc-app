...
def load_data():
    ...
    try:
        ...
        st.success("✅ Showing only your key secured parties")
    except:
        df = pd.read_sql("SELECT * FROM ucc_filings ORDER BY Ucc1FilingNumber DESC", conn)
        st.warning("⚠️ Using full database — Run 'python3 refresh_key_parties.py' to filter")
    conn.close()
    return df

df = load_data()

# Debtor left → Secured right
desired_order = [
    'Ucc1FilingNumber',
...
# ====================== TAB 3: EQUIPMENT FINANCING (kept because you like it) ======================
with tab3:
    st.subheader("🏗️ Equipment Financing Search")
    st.markdown("**Quick search for construction & industrial equipment liens**")
    keywords = ["Excavator", "Crane", "Loader", "Bulldozer", "Forklift", "Caterpillar", "John Deere"]
